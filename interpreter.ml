...
let rec interpret_expr env e =
  ...
  (* Voici deux cas en exemple, à vous d'écrire les autres ! *)
    ...
    | Eprint_int e ->
       ...
       let i = get_int e.pos v in
       print_int i ;
       Vunit
    
    |Ebinop (( Beq  | Bneq | Blt  | Ble  | Bgt  | Bge
                                   | Badd | Bsub | Bmul | Bdiv
                                   | Band | Bor ) as op, e1,e2)  ->  
       let i1 = get_int e1.pos  (interpret_expr env e1) and 
              i2 = get_int e2.pos  (interpret_expr env e2)  in
              Vint(match op with
                   |Beq 
                   |Badd -> i1 + i2
                   |Bsub -> i1 - i2
